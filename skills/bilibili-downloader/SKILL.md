...
### 6. 视频支持在iphone/ipad播放

```bash
python ~/.claude/skills/bilibili-downloader/bili_download.py --url "BV链接" --iphone
```

### 7. cookies来源指定文件

```bash
python ~/.claude/skills/bilibili-downloader/bili_download.py --url "BV链接" --cookies "cookies.txt"
```
